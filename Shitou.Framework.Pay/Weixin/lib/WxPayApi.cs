...
namespace Shitou.Framework.Pay.Weixin
{
    public class WxPayApi
    {
        ...
        /// <param name="timeOut">超时时间</param>
        /// <returns>成功时返回订单查询结果，其他抛异常WxPayException</returns>
        public static WxPayData OrderQuery(WxPayData inputObj, int timeOut = 6)
        {
            ...
            queryData.SetValue("out_trade_no",inputObj.GetValue("out_trade_no"));
            queryData.SetValue("transaction_id", inputObj.GetValue("transaction_id"));
            queryData.SetValue("appid", WxPayConfig.APPID);//公众账号ID
            queryData.SetValue("mch_id", WxPayConfig.MCHID);//商户号
            queryData.SetValue("nonce_str", GenerateNonceStr());//随机字符串
            queryData.SetValue("sign", queryData.MakeSign());//签名

            string xml = queryData.ToXml();
        ...
        /// <returns>成功时返回API调用结果，其他抛异常WxPayException</returns>
        ...
        /// <param name="timeOut">超时时间</param>
        /// <returns>成功时返回接口调用结果，其他抛异常WxPayException</returns>
        ...
        /// <param name="inputObj">提交给申请退款API的参数</param>
        /// <param name="timeOut">超时时间</param>
        /// <returns>成功时返回，其他抛异常WxPayException</returns>
        ...
        /// <returns></returns>
        public static SortedDictionary<string, object> GeneratePayData(string prepay_id)
        {
            WxPayData payData = new WxPayData();
            payData.SetValue("appid", WxPayConfig.APPID);//公众账号ID
            payData.SetValue("partnerid", WxPayConfig.MCHID);//商户号
            payData.SetValue("noncestr", GenerateNonceStr());//随机字符串
            payData.SetValue("timestamp", GenerateTimeStamp());//时间戳
            payData.SetValue("package", "Sign=WXPay");
            payData.SetValue("prepayid", prepay_id);
            payData.SetValue("sign", payData.MakeSign());
            return payData.GetValues();
        }
    }
}