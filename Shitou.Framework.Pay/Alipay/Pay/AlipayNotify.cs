...
namespace Shitou.Framework.Pay.Alipay.Pay
{
    ...
    public class AlipayNotify
    {
        ...
        public bool Verify(Dictionary<string, string> inputPara, string notify_id, string sign)
        {
            //获取返回时的签名验证结果
        ...
        private string GetPreSignStr(Dictionary<string, string> inputPara)
        {
        ...
        private bool GetSignVeryfy(Dictionary<string, string> inputPara, string sign)
        {
            ...
            //获取待签名字符串
            string preSignStr = AlipayCore.CreateLinkString(sPara);
            LogHelper.SaveFileLog("SignStr", preSignStr + "&sign=" + sign);
            //获得签名验证结果
            bool isSgin = false;
            if (sign != null && sign != "")
            {
